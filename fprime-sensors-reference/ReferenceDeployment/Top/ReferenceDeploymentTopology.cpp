...
#include <fprime-sensors-reference/ReferenceDeployment/Top/ReferenceDeploymentTopologyAc.hpp>
// Note: Uncomment when using Svc:TlmPacketizer
//#include <fprime-sensors-reference/ReferenceDeployment/Top/ReferenceDeploymentPacketsAc.hpp>

// Necessary project-specified types
#include <Fw/Types/MallocAllocator.hpp>
#include <Svc/FrameAccumulator/FrameDetector/FprimeFrameDetector.hpp>
#include <fprime-sensors-reference/ReferenceDeployment/Top/Ports_ComPacketQueueEnumAc.hpp>
...
void configureTopology(const TopologyState& state) {
    ...
    Svc::BufferManager::BufferBins gpsBufferMgrBins;
    memset(&gpsBufferMgrBins, 0, sizeof(gpsBufferMgrBins));
    gpsBufferMgrBins.bins[0].bufferSize = 1024;
    gpsBufferMgrBins.bins[0].numBuffers = 1;
    gpsBufferMgrBins.bins[1].bufferSize = 1024;
    gpsBufferMgrBins.bins[1].numBuffers = 4;
    NmeaGps::bufferManager.setup(BUFFER_MANAGER_ID, 0, mallocator, gpsBufferMgrBins);

    // Frame accumulator needs to be passed a frame detector (default F Prime frame detector)
    NmeaGps::frameAccumulator.configure(nmeaDetector, 1, mallocator, 2048);

    NmeaGps::driver.open(state.gpsDevice,
                         static_cast<Drv::LinuxUartDriver::UartBaudRate>(state.gpsBaud),
...
void setupTopology(const TopologyState& state) {
    ...
    Os::TaskString name("NmeaGpsReadTask");
